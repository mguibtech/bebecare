/**
 * Handler de eventos do notifee em background/quit para os alarmes de remédio.
 *
 ...
 * launchActivity 'default'), entao não ha trabalho a fazer aqui. Fica como
...
export function registerAlarmBackgroundHandler(): void {
  ...
  // ainda não estiver no build (antes do primeiro rebuild apos instalar), a
  // chamada lancaria e derrubaria o boot. O try/catch mantem o app vivo.
  try {
    notifee.onBackgroundEvent(async ({ type, detail }) => {
      // Placeholder: PRESS já abre a MainActivity via launchActivity.
      // Acoes de notificação (snooze/tomar) serao tratadas aqui.
